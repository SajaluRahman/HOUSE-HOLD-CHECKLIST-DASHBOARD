...
const handleExport = async ()=>{
    ...
    title.value = "HOUSEKEEPING AUDIT CHECKLIST";
    title.font = {
        size: 18,
        bold: true
    };
    ...
    sheet.addRow([]);
    // ======== MEASURABLE STANDARD ========
    sheet.mergeCells(`A8:F10`);
    const std = sheet.getCell("A8");
    std.value = `Measurable Standard: Clean = dust free, tidy & spotless.\n` + `KEY: (✓) Met    (✗) Not Met    (N/A) Not Applicable`;
    std.font = {
        size: 10,
        italic: true
    };
    ...
    // ======== CATEGORY + ITEMS (Matches PDF Format) ========
    filteredCategories.forEach((category, index)=>{
        // Category Header Row
        const catRow = sheet.addRow([
            `${index + 1} ${category.name.toUpperCase()}`
        ]);
        ...
        category.items.forEach((item)=>{
            const row = sheet.addRow([
                ...
                item.score || "",
                item.frame || "",
                item.status === "checked" ? "Y" : item.status === "crossed" ? "N" : "N/A"
            ]);
            row.height = 20;
            row.eachCell((cell)=>{
                cell.border = {
    ...
    const checked = allItems.filter((i)=>i.status === "checked").length;
    const summaryRow = sheet.addRow([
        `SCORED: ${checked} / ${allItems.length}  (${Math.round(checked / allItems.length * 100)}%)`
    ]);
    ...
    a.href = url;
    a.download = `Housekeeping_Audit_${Date.now()}.xlsx`;
    a.click();
...
//# sourceMappingURL=_df799c7f._.js.map